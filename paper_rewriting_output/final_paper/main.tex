...
\begin{document}
\maketitle
\begin{abstract}\hypertarget{resumo}{%
\label{resumo}}
...
\audit{C055, C057}{E032}; banda de dose A6 no degrau humano: 0,0--2,6 µg
de V127ΔGPI por depósito (pior caso κ=8: 0,2--10,3 µg; redose ≤7 d; MW
22,83 kDa das sequências próprias) --- a largura da banda ≈53× é o
achado, dominada pelo proxy de Kd, até o braço G0-A6 fechá-la
\audit{C058–C060}{E057, E058}; registro de 60 claims, 58 fontes, 65
fatos numéricos com validação por máquina. \textbf{Conclusão.} A tese
está realizada --- \textbf{e é uma etrização}: entregue, rotulada,
auditável --- nos termos da etrização: continuar pesquisa por simulação
parametrizada sem substituir o laboratório --- se dados reais futuros
forem análogos aos simulados, os passos seguintes já estarão avançados
(antecipação bancada).
...
\hypertarget{capuxedtulo-2-introduuxe7uxe3o}{%
\chapter{CAPÍTULO 2 ---
...
\subsection{2.1 Problema}\label{problema}}
...
\subsection{2.2 Justificativa}\label{justificativa}}
...
\subsection{2.3 Questões de pesquisa}\label{questuxf5es-de-pesquisa}}
...
\subsection{2.4 Objetivos}\label{objetivos}}
...
linhagem completa dos dados (Cap. 7) \audit{C046}{E032, E033}. -
...
\subsection{2.5 Hipóteses}\label{hipuxf3teses}}
...
\subsection{2.6 Estrutura do documento}\label{estrutura-do-documento}}
...
\subsection{2.7 Componentes M1--M5 (tabela validada pela autora) e
natureza do
...
\hypertarget{capuxedtulo-3-fundamentauxe7uxe3o}{%
\chapter{CAPÍTULO 3 ---
...
\subsection{3.1 Doenças priônicas: o gargalo terapêutico que motiva a
...
\subsection{3.2 A família dos métodos antecipatórios e os in-silico
...
\subsection{3.3 Posicionamento da etrização (corroborando
...
legitima} --- e ao legitimá-lo com critérios (5 requisitos da §5.1), o
disciplinariza.
...
\subsection{3.5 Síntese}\label{suxedntese}}
...
\hypertarget{capuxedtulo-4-base-comum-de-dados-a-ponte-entre-fundamento-e-aplicauxe7uxe3o}{%
...
\section{4.1 A mesma base sob os dois
...
molecular usada na dose (22,83 kDa \audit{E}{E058}).

\hypertarget{cronologia-honesta-o-examinador-hostil-perguntaria-respondemos-antes}{%
\section{4.2 Cronologia honesta (o examinador hostil perguntaria ---
respondemos
...
\section{4.3 Tabela de validação (todos os números dos JSONs do
...
contém até Kt≈2 (ressalva de horizonte, §5.4) \\
...
\hypertarget{capuxedtulo-5-fundamento-a-inariuxe2ncia-de-ux3b8-validauxe7uxe3o-multi-espuxe9cie}{%
...
\section{5.1 Do sweep de composição à parametrização por
...
\section{5.2 Cenário B {[}SIM{]}}\label{cenuxe1rio-b-sim}}
...
\section{5.3 Regra de titulação
...
\section{5.4 Dependência de horizonte (a única assimetria
...
\begin{figure}
\centering
\includegraphics[width=0.92\textwidth]{../../paper/latex/figs/fig4_theta_species.png}
...
\hypertarget{capuxedtulo-6-aplicauxe7uxe3o-o-desenho-terapuxeautico-emerge}{%
...
\section{6.1 Transporte: três regras falseáveis
...
\section{6.2 O limiar travado e a dose legitimada pela
...
\section{6.3 A primeira dose calculada: banda de dose do braço A6
...
ilustrativa da Parte 1 §2.2 \emph{(não é estimativa de secreção
...
Da). Fontes da cadeia: Kd-piso \audit{E}{E057} · V-halo \audit{E}{E030}
· ECS \audit{E}{E010} · MW \audit{E}{E058} · titulação \audit{E}{E032}.
...
\begin{figure}
\centering
\includegraphics[width=0.92\textwidth]{../../paper/latex/figs/fig5_dose_ladder.png}
...
\section{6.4 O ensaio que fecha a banda: G0-wet
...
\hypertarget{capuxedtulo-7-muxe9todos-a-etrizauxe7uxe3o-formalizada}{%
\chapter{CAPÍTULO 7 --- MÉTODOS: A ETRIZAÇÃO
FORMALIZADA}\label{capuxedtulo-7-muxe9todos-a-etrizauxe7uxe3o-formalizada}}

\hypertarget{pipeline-do-muxe9todo-p0p6}{%
\subsection{7.0 Pipeline do método
(P0--P6)}\label{pipeline-do-muxe9todo-p0p6}}
...
\emph{Tabela completa com entradas/saídas/garantias por passo: §7.1.}

\hypertarget{o-muxe9todo-nomeado-etrizauxe7uxe3o-computacional-passos-p0p6}{%
\section{7.1 O método nomeado: etrização computacional, passos
...
\section{7.2 Formulação matemática com
...
§4.2 & humano (Aβ42-PrP; PROXY declarado) & --- & cadeia dimensional
...
\section{7.4 A tese em forma experimental (mapeamento
...
\hypertarget{capuxedtulo-8-resultados-como-validauxe7uxe3o-componentes-m1m5}{%
...
8--12 mm; malha ≥5×r\_p; redose ≤7 d) & \audit{C033, C034, C035}{} \\
...
V-halo) & \audit{C058–C060}{E057, E058} \\
...
\hypertarget{capuxedtulo-9-achados-impactos-e-uxe1reas-correlatas-pruxf3ximas-abordagens}{%
...
\subsection{9.1 Achados e impactos}\label{achados-e-impactos}}
...
\subsection{9.2 Áreas correlatas com a mesma
...
\subsection{9.3 Próximas abordagens (declaração mandatória conforme
...
\hypertarget{capuxedtulo-10-discussuxe3o}{%
...
\subsection{10.1 Promessas e limites desta Parte
...
\subsection{10.2 Discussão: o que os resultados significam e o que não
...
\hypertarget{capuxedtulo-11-camada-cluxednica-pruxe9-abordagem-dos-temas-antes-da-penetrauxe7uxe3o-tuxe9cnica}{%
...
\hypertarget{resumo-em-1-puxe1gina-rota-de-leitura-de-10-minutos}{%
...
\hypertarget{acompanhamento-previsto-no-desenho-sim-documentauxe7uxe3o-de-desenho-nuxe3o-conduta-cluxednica}{%
...
\hypertarget{capuxedtulo-12-limitauxe7uxf5es-como-fruto-cada-limite-com-o-que-o-fecha}{%
...
\hypertarget{capuxedtulo-13-conclusuxf5es-por-objetivo}{%
...
\audit{C038, C051, C052, C055–C057, C058–C060}{}.
...
\textbf{CAMADA 0 --- Fonte primária (papers publicados):} Asante 2015
{[}E001{]} (Nature, ``V127 resiste a príons'') · Mead 2009 {[}E002{]}
(NEJM, ``G127V seleção kuru'') · Groveman 2019 {[}E007{]} (Acta
Neuropathol, ``Organoides infectam com sCJD'') --- peer-reviewed.

→ \textbf{CAMADA 1 --- Verificação (auditoria de fonte):} cada fonte foi
ABERTA (URL/DOI confirmado); identificadores verificados → E-registry;
status \emph{verified} (quem abriu, quando, como).

→ \textbf{CAMADA 2 --- Claim registry (afirmações extraídas):} C001
``V127/V127 resiste completamente'' {[}E001{]} · C003 ``DN
dose-dependente'' {[}E001,E003,E005{]} · C005 ``anchorless trans DN''
{[}E003{]} · C010 ``âncoras de relógio 25-28/35/169 dpi'' {[}E007{]} ·
C011 ``títulos MV2 vs MV1 @169dpi'' {[}E007{]}.

→ \textbf{CAMADA 3 --- Parâmetros (números com proveniência):} α=0,20 e
λ=1,8 ← Thorne \& Nicholson 2006 {[}E010{]} (in-vivo, humano) ·
D₀=1,25×10⁻¹⁰ m²/s ← Stokes-Einstein (R\_h≈2,5 nm, \textasciitilde30
kDa) · D\_eff=3,86×10⁻¹¹ ← D₀/λ² (derivado) · k\_eff=10⁻⁶--10⁻⁵ ← Masel
1999 {[}E011{]} · t\_dupl=12,1 d ← 134d/log₂(2130) {[}E007{]} · 1
unidade=144 d ← t\_dupl/t\_dupl\_sim {[}E032{]}.

→ \textbf{CAMADA 4 --- Modelo matemático (equações com parâmetros):}

\[\frac{\partial(\alpha c)}{\partial t} = \nabla\!\cdot\!(D_{\mathrm{eff}}\,\nabla c) \;-\; \nabla\!\cdot\!(\mathbf{v}\,c) \;+\; S(\mathbf{x}) \;-\; k_{\mathrm{eff}}\,c\]

→ solver WS-7 (volumes finitos, 192², auto-testado) ·
freeS(r)=(1+κ·c\_V127(r))⁻² → modelo WS-9 (campo-médio, 96², humanizado)
· θ≡(1+κ·c\_pico)⁻¹ → definição formal (a fração de replicação que
resta). → \textbf{CAMADA 5 --- Execução {[}SIM{]}:} sweeps (S1/S2/S3),
grade κ, calibração do estimador; runs arquivados com hash; reprodução
em 2 ambientes.

→ \textbf{CAMADA 6 --- Colheita com critérios pré-declarados:} veredito
ADEQUADO na fronteira; refutações reportadas (hamster; v1.1).

→ \textbf{CAMADA 7 --- Prognósticos travados por release:} θ*=0,333
(v1.0); regras 1-3; banda A6 (C058-C060).

→ \textbf{CAMADA 8 --- Conclusões da tese:} método (etrização P0-P6) +
prognóstico (θ*, regras) + dupla potencialidade (cura + restauração
{[}C016{]}) + ressalva (não aplicar em humanos sem validação
experimental).
...
DECLARA isto na Base de Validade (§5.3): ``não substitui; estimula P\&D
ágil'' \\
