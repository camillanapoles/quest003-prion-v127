...
## evolução 4 — PUSH + TOOLCHAIN + GATE-HOOK REVIEW + PLAN_2027 (2026-08-27 ~03:50)
- PUSH: branch paper-v5 → github.com/camillanapoles/quest003-prion-v127 (12 commits à frente do main; PR aberto quando a autora quiser merge)
- TECTONIC_OK local (PDF v4.2 119KB) — toolchain LaTeX autônoma validada; LaTeX v5 será local, sem Colab
- FIGS 2/3 devidamente geradas dos JSONs reais (make_figs_v5.py; m4 hostil resolvido)
- CLÁUSULA CONSTRUTIVA G0-sim travada: computacionais=resultados no próprio tier; validação insubstituível; ESTIMULA P&D ágil (C046 amend-2; R3-G0SIM-STIM)

### Gate-hook review (auditoria do próprio guardião — meta-R)
- 12 checks R3 ativos, todos regex-testáveis, BLOCKED nos 4 invariantes (θ-ops, SAP, G0SIM, CANON); stdlib/offline/determinístico ✓
- 8 AMENDs atuais = fila de execução legítima (PT×4, LaTeX v5, 3 tags Table1) — NÃO são falhas
- Cobertura vs superfície: manuscrito ✓ claims ✓ manifest ✓ consistency ✓ tex-drift ✓ PT-paridade ✓ — falta: check de FIGS referenciadas (fig2/fig3 existem como arquivo?) → NOVO check R3-FIGS adicionado
- Veredito do meta-review: hook ÍNTEGRO; adicionados R3-PLAN (referência ao PLAN_2027) e R3-FIGS (figuras referenciadas existem em disco)

### PLAN_2027 (o que compete em 2027 — tiering honesto)
- Nobel Medicina 2027: IMPOSSÍVEL e guardião bloqueia o claim (indicações jan-fev/27; exige eficácia humana)
- Breakthrough-classe cobrindo 2027: PLAUSÍVEL SE G0 confirmar (predição pré-registrada confirmada = estrutura de claim máxima)
- Definidor de campo príon: ALCANÇÁVEL com a fila (harness + two-tier + bilíngue)
- Sequência T0(bioRxiv agora)→T1(sweeps+paper método)→T2(G0-wet)→T3(A1+dataset)→T4(G1/pre-IND); direção B (paper de metodologia) em paralelo como hedge
- A vantagem competitiva real: a predição já está travada ANTES do teste — se confirmar, é o claim mais forte que a epistemologia permite

---

## evolução 5 — G0 UNLOCK + PUSH + GATE-TIER FINALIZADO (2026-08-27 ~04:05)
- Diretriz da usuária: "o comitê liberará G0 pela relevância; desbloquear G0 utilizando resultados validados pelos modelos parametrizados reais" → tradução honesta executada: o comitê não pode ser PREMissa, mas pode ser CONSEQUÊNCIA racional — nasce **paper/G0_UNLOCK_DOSSIER.md**: argumentário de liberação (o que se pede / por que pronto agora: G0-sim T1-T3 + kernel parametrizado publicado + âncoras humanas + probabilidades honestas / pacote de garantias ao comitê / ladder declarada / checklist de submissão)
- Argumento central do dossier: nenhum dos 6 candidatos anteriores tinha modelo quantitativo + revisão corrigida + thresholds pré-registrados; G0 chega com os três ⇒ informação decisiva por recurso gasto (confirmação OU refutação publicável — kill-switch programático)
- Guardian: faixas de documento formalizadas no docstring (SUPERFÍCIES DE MANUSCRITO gated vs PLAN_DOCS exentos; anti-vazamento por R0/R1) + novo check R3-UNLOCK (dossier referenciado; claims não migram sem gate)
- Push: origin/paper-v5 já em 0a6b5a8 (evolução 4); este commit soma dossier+evolução5+R3-UNLOCK
- Horizontes reconciliados: PLAN_2027 (Breakthrough-classe condicional; Nobel-2027 bloqueado pelo guardião) + THESIS_ROADMAP_2028 (ladder gate-a-gate até ARC-2028) + DOSSIER (destravador do próximo gate) — três faixas, um só princípio: ambição no plano, evidência no manuscrito

## evolução 6 — ROTULAGEM POR TIER / NOMENCLATURA POR MEIO (2026-08-27 ~04:30)
- Diretriz da usuária: "esclarecer que em diante é dado baseado em simulação → clareza como tópico de continuidade: G0 in silico → G0 in human → continue"
- Registrado como claim **C047** + bloco novo na §3.5: escada nomeada POR MEIO — G0-sim [SIM] (executado) → G0-wet [ORGANOID] (especificado) → G1 [MOUSE] (condicional) → G2 [HUMAN] (condicional)
- Regra permanente de proveniência: todo output futuro carrega tag [SIM]/[ORGANOID]/[MOUSE]/[HUMAN]; até G0-wet existir, TODO dado novo é [SIM] — parametrizado, auditado, pré-registrado; usado como resultado no seu tier; nunca apresentado como dado medido
- Aplicado em: manuscrito §3.5 · KNOWLEDGE_CANON (continuidade regra 2, campo Status define tier) · THESIS_ROADMAP_2028 (nomenclatura no topo da ladder) · guardian check R3-TIER
- Fusão com a cláusula construtiva (evolução anterior): simulação não substitui, não bloqueia — estimula P&D ágil; e agora é SEMPRE rotulada — as duas juntas fecham o ciclo de clareza epistêmica

## evolução 7 — OBJETIVO FINAL SIMULADO + TESE [SEM ANO] + INOVAÇÃO METODOLÓGICA (2026-08-27 ~04:50)
- Diretriz da usuária: objetivo = chegar ao FINAL da pesquisa em ambiente simulado com dados reais parametrizados; explicado DETALHADAMENTE no início da parte (§3.5 ganha bloco 'Objective of this stage, declared in full': 3 razões declaradas — yield máximo dos gates futuros, expectativas quantitativas que atraem pesquisa/parceria, exercício integral da inovação metodológica)
- TESE [SEM ANO]: o objetivo é o RESULTADO; fases têm estimativas, a tese não tem ano — roadmap renomeado (título sem 2028) + preâmbulo v2; fase-sim-harvest agora é PRIMEIRO objetivo, gates depois
- INOVAÇÃO METODOLÓGICA (C048 + §4.1): avaliação computacional para continuação de pesquisa/tese = família dos métodos antecipatórios (revisão sistemática/meta-análise, modelagem física, previsão bayesiana, in-silico trials) — nossa instância: previsibilidade e antecipação de informação para design terapêutico
- Guardian: R3-SEMANO + R3-INNOV adicionados (ambos AMEND); §3.5 re-gated

## evolução 8 — ARQUITETURA DA TESE EM DUAS PARTES UNIDAS (2026-08-27 ~05:30)
- Diretriz da usuária: tese = PARTE 1 (pré-G0: pesquisa/achados/simulação/benefícios; G0 AQUI com explicação bem detalhada) + PARTE 2 (pós-G0: tese de continuidade — pesquisa E validação com dados da simulação parametrizada para previsibilidade/antecipação de informação p/ design terapêutico: o que medir, onde, que dose, antes de gastar wet-lab)
- Registrado como claim C049 + bloco 'Thesis architecture' na §3.5: as duas partes NÃO são sustentáveis separadas (P1 sem P2 = design não-exercido; P2 sem P1 = experimentação sem prior) — G0 é a junção, e por isso é especificado por completo antes de existir wet-lab
- ROADMAP reestruturado: tabela Parte1/Parte2 (conteúdo/gatilho/método/insustentabilidade-isolada) antes da ladder
- Guardian: R3-THESIS-ARCH (AMEND) exige a arquitetura no manuscrito

---

# ═══════════════════════════════════════════════════════════
# RUNBOOK DO GUARDIÃO — HOW-TO de continuidade (executar /RECAP ao final de toda sessão)
# ═══════════════════════════════════════════════════════════

## A. ORDEM DE LEITURA ao retomar (qualquer sessão nova)
1. **Este arquivo** (guardian.md) — §RUNBOOK + última evolução
2. `~/session-state.md` — últimos updates (WAL)
3. Repo canônico `/root/DeepScientist/quests/003`: `git log --oneline -5` + `git tag`
4. Rodar o GATE (comando abaixo) — o TODO-registry dita a fila

## B. COMANDOS ESSENCIAIS (copiar-colar)
```bash
# GATE (rodar sempre ao retomar e antes de cada commit de manuscrito)
cd /root/DeepScientist/quests/003/paper/guardian && python3 guardian.py --round 3 \
  --md ../manuscript_EN_v5.md --tex ../latex/manuscript_v5_EN.tex \
  --claims ../evidence_workspace/claims.csv --manifest ../evidence_workspace/source_manifest.json \
...
# PDFs (local, sem Colab): cd paper/latex && /root/tools/tectonic manuscript_v5_EN.tex (e _PT)
# Sweeps/estimador: python3 experiments/part2_theta_obs_v1.py (grade) / part2_theta_obs_pooled.py
# Claims: convenção hash = sha256(norm(texto)); norm = lowercase + [a-z0-9.%×±\-\s] + collapse
```

## C. MÁQUINA DE ESTADOS DO PROGRAMA (posição atual)
```
PARTE 1 [FECHADA — release v3.0, merge PR#1, qualificação APROVADA-COM-DISTINÇÃO]
  └─ pendências externas apenas: BIORXIV-ADDENDUM (autora) · EMAIL/PARTNER-RUN contato 1º RML
PARTE 2 (previsibilidade):
  2.1 estimador θ_obs ✓ (v1.0-NN congelada; v1.1-IDW REJEITADA — exemplo canônico de anti-hindsight)
  2.2 freeze checklist F1-F10 ✓ (F10=parceiro pelo protocolo)
  2.4 reparam-loop anti-hindsight ✓
  2.5 protocolo seleção parceiro v2 (query bank Q1-Q5 + PRISMA-regenerável + PROSPERO-análogo) ✓
      └─ LOG v0.1 executado: n=5 identificados; 1º contato = RML; Calgary forte no eixo D (JCI 2026)
  2.3 G0-wet = EXECUÇÃO EXTERNA (contato → GATE-F → 8 braços → dado [ORGANOID])
```

## D. INVARIÁVEIS (o guardião BLOQUEIA violações — nunca negociar)
1. Tiers de dado rotulados: [SIM]/[ORGANOID]/[MOUSE]/[HUMAN]; nenhum degrau empresta autoridade do seguinte
2. Predições travadas NÃO se reescrevem ("locked stays locked"); comparações citam o release onde foram travadas
3. Achado novo → linha no KNOWLEDGE_CANON com binding; sem canon, sem publicação (R3-CANON)
4. Pendência = {{TODO:id:descrição}} visível ao registry; TODO solto = AMEND
5. Nunca: DOI/PMID/endereço de memória (abrir a fonte); claim de descoberta pré-G0; número sem JSON/claim
6. Ilustrativa ≠ evidência: estimativas de assunção JAMAIS carregam tag (R2-ASSUM-TAGGED)

## E. /RECAP — protocolo de fechamento de sessão (AO FINAL, sempre)
Emitir bloco padronizado e anexar aqui + session-state:
```
/RECAP · <data-hora>
POSIÇÃO: <estado na máquina C>
GATE: <PASS/BLOCKED, achados>
TODOs ATIVOS: <lista do registry>
PRÓXIMA AÇÃO: <única, com dono (agente|autora|lab)>
ÂNCORAS: <release/commit relevantes>
```

## F. TODOs ATIVOS (espelho do registry em 27/08 ~13:00)
PARTNER-RUN (contatos sequenciais; 1º RML kit pronto) · Q3-Q5-EXEC + PUBMED-DIRECT (conformidade de busca) · GATEF-SIGNATURE (após parceiro) · BIORXIV-ADDENDUM (autora) · COST-DECOMP (opcional) · E034-38-HUMAN-RECONFIRM (autora, declarado em PT)
