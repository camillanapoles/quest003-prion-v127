...
## §P-003 — Elevação claim-a-claim (01/09, pós-merge PARTE 3)
- Base: 82 candidatos (batches 1-6, branch executor) com verificação consolidada (17 exato + 21 fuzzy + 16 URL + 5 dedup + 2 diverge-ano).
- **Verificação independente do agente**: esummary + title-overlap ≥0,50 + ano ±1 → 7 PMID-verificados (overlap 0,8-1,0; **0 mismatch**) + 10 DOI-exatos herdados do crossref-do-executor (proveniência: OPTIMIZATION_LOG D-02/D-03).
- **Elevação: 39 → 56 fontes (E040-E056)** · validate_manifest 0/0 · check_references 0/0 · dedupe contra as 39 (0 colisões) · E049/E055 com year=null (SSRN/preprint — aceitos pelo schema; documentado).
- Fuzzy não-elevados (14 sem PMID resolvível) e URL/institucionais (16) documentados no pool (`experiments/xspecies/p003_{pool,mycheck}.json`) — futura elevação exige identifier resolvível.
- Bindings (48 rascunhos) permanecem rascunhos — mapeiam destino na tese-otimização (condicionada a P-009 da autora); nenhuma claim nova criada (nenhuma superfície gated tocada).
