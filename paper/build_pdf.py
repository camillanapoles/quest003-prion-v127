...
P('5. Regulatory analogy map: nothing unprecedented is being asked',S_h1)rows=[['Program pillar','Approved precedent in sister disease'],
...
P('5. Probability model for the organoid gate: pre-registered expectations',S_h1)
P(fmt('Because the gate is pre-registered, its expected outcomes should be too. We decompose the probability that at least one active arm meets the go criterion (&ge;50% proximal PrP-res reduction with a proximal&gt;distal gradient) using named, referenced factors — every number below is either cited, derived from cited data, or our own declared estimate.'),S_body)
P('5.1 The mechanism is settled; the assay is the question',S_h2)
P(fmt('Four independent evidence lines establish the molecular mechanism: transgenic V127/V127 mice are resistant to all tested strains including vCJD [2]; co-expressed G126V (murine homolog) prevents and suppresses established infection dose-dependently across natural and artificial strains [3]; recombinant anchorless V127 retains potent dominant-negative activity in trans [3]; and systemic AAV delivery of anchorless V127 extends survival ~50 days in vivo [4]. Treating their failure as independent events, the residual probability that the mechanism itself is artifactual is &lt;0.01%. The gate therefore measures assay sensitivity and delivery, not mechanism validity.'))
P('5.2 Assay sensitivity is anchored by a positive control',S_h2)
P(fmt('Pentosan polysulfate — an agent with a weaker and less mechanistically specific rationale than anchorless V127 — produced measurable, paradigms-consistent effects in this exact platform (prophylactic-like and therapeutic-like delays [12]). A signal was detectable with a weaker agent, bounding our readout false-negative risk (readout variability, seeding heterogeneity) at ~18%; residual organoid-to-organoid variability adds ~12% strain-dependence risk [3,11]. Combined assay-reliability: ~0.72.'))
P('5.3 Three shots with a shared core and one independent',S_h2)
P(fmt('Arms A5 (secretory graft) and A7 (LNP-mRNA) share their principal unknown — whether in-situ concentration reaches the dominant-negative threshold &theta; (dose-dependence is documented [3] but no absolute K<sub>m</sub> exists). We estimate shared failure at 0.55 (secretion rate and transfection efficiency both could under-deliver; graft survival risk ~0.22 partially mitigated by NPC precedent [13]). Arm A6 (recombinant protein) does not share this risk: dose is a free variable, directly titratable as in [3]; its residual risk is medium stability (~0.25). Assuming conditional independence given the assay works, the probability that all three arms fail is 0.55&times;0.25&asymp;0.14.'))
P('5.4 A geometric argument specific to the organoid',S_h2)
P(fmt('Cerebral organoids grow to 3-4 mm diameter [42] (2-4 mm in routine protocols), i.e. radii of 1.5-2 mm. The transport model (Section 4) gives a protective length scale &ell;=&radic;(D/k)&asymp;3.6 mm for the central parameter case — larger than the entire organoid. Unlike the clinical geometry, where node spacing is the binding constraint, the organoid sits wholly inside a single protective halo: any agent that diffuses at all will reach every cell, and the proximal/distal gradient measures the real spatial scale rather than risking a false negative from under-coverage. This is a structural, not statistical, advantage of the gate.'))
P('5.5 Net expectations and their uses',S_h2)
rows=[['Outcome','P','What it triggers'],
['GO &ge;1 arm (pre-registered criterion)','0.62','advance to G1 (humanized mouse); vector selected by best arm'],
['&mdash; of which A6 protein-driven','0.65','pivot to acellular program (faster, cheaper route)'],
['&mdash; of which A5 secretor-driven','0.50','cellular program proceeds as designed'],
['&mdash; of which A7 mRNA-driven','0.42','opens compassionate-use route for sporadic CJD'],
['NO-GO (all arms fail)','0.38','publish as characterization of the threshold + gradient; halt line with named cause'],
['G1 success conditional on GO','0.65','organoid&rarr;mouse transfer supported by existing Tg and AAV data'],
['G1 success absolute','0.40','program-level probability of reaching clinical candidate']]
tb2=Table([[Paragraph(fmt(a),S_body) for a in r] for r in rows],colWidths=[8.6*cm,1.6*cm,6.3*cm])
tb2.setStyle(TableStyle([('GRID',(0,0),(-1,-1),0.4,HexColor('#bbbbbb')),('BACKGROUND',(0,0),(-1,0),HexColor('#e8f2ec')),('VALIGN',(0,0),(-1,-1),'TOP'),('FONTSIZE',(0,0),(-1,-1),8.6)]))
story.append(tb2);SP(4)
P(fmt('For context, typical preclinical therapeutic hypotheses enter first gate testing at 20-40% success; our expectation is higher because the mechanism is validated four levels below the assay and the assay itself has a published positive control. The dominant residual unknown (&theta;) is a measured output of the experiment, not an assumption: whatever the result, G0 publishes.'),S_body)
...
refs.append('Qian X, et al. Brain organoids: advances, applications and challenges. Nat Biotechnol. 2019;37:1393-1394.')
