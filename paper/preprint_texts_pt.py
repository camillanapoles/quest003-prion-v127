...
PT={
 ...
 's7':[
  ...
  'O halo de proteção de um depósito (quantificado na Tabela 2) segue o clássico problema de Thiele da engenharia química: comprimento de penetração ℓ=√(D/k). Para o caso médio (k=3×10⁻⁶ s⁻¹), ℓ≈3,6 mm e r₁₀% = 4,2-5,8 mm por nó; o regime estacionário estabelece em ~4 dias, de modo que leituras de 90 dias no organoide operam em equilíbrio profundo.'],
 ...
 'decl':[
  ...
  '**Contribuições de autor (CRediT):** Camilla N. — conceitualização, metodologia, software, investigação (simulações), redação (rascunho original). Conforme SW-S01/SW-03, sistemas de IA não são autores; todas as decisões científicas, a verificação-na-fonte das cinco referências críticas (Asante 2015; Gatdula 2026; Zerbes 2026; Groveman 2019/2021) e a aprovação final pertencem à autora humana responsável (registro na trilha de auditoria do repositório).',
  '**Divulgação de uso de IA:** ferramentas assistidas por IA foram usadas sob instrução humana explícita para recuperação de literatura, execução de código, porting de simulação e geração de rascunho; toda alegação foi verificada na fonte ou carrega status de não-verificada declarado; simulações são determinísticas e reprodutíveis dos scripts publicados.',
