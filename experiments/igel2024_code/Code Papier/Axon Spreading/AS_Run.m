...
while t<t_lim && sum(PART,'all')
    ...
    for n=1:N_neur      % Update UPR
        ...
        NEUR(n,4) = max(NEUR(n,4),(c>uprd(NEUR(n,2)))*uprt(NEUR(n,2)));
        if xor(~NEUR(n,3),~NEUR(n,4))   % if neuron changes state, add elems of tpl zone to updm
            updm = [updm N2M{n,1}];
        end
    end
    ...
    for m=updm
        tp = sum(NEUR(cellfun(@(x) any(x==m),N2M(:,1)),4)==0);
    ...
    NEUR(:,3) = NEUR(:,4);
    NEUR(:,4) = max(NEUR(:,4)-tau,0);   % Update upr based on tau
    UPR = UPR + [NEUR(:,4)'>0 1]*tau;   % UPR tracking
